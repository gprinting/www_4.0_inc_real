...
class NCR extends Product implements PrintoutInterface
{
    /**
     * @var string
     */
    ...
    function setInfo($param)
    {
        ...
        $this->diff_yn    = $param["diff_yn"] == "true" ? true : false;

        $this->paper_mpcode_arr    = $param["paper_mpcode_arr"];
        if(!is_array($this->paper_mpcode_arr)) {
            $this->paper_mpcode_arr = explode(",",$this->paper_mpcode_arr);
        }
    ...
    function cost() {
        ...
        $paper_mpcode_arr_count = count($this->paper_mpcode_arr);
        $is_fst = true;
        for ($i = 0; $i < $paper_mpcode_arr_count ; $i++) {
            $paper_mpcode = $this->paper_mpcode_arr[$i];
        ...
        $sum_paper_price *= $this->discount_rate;
        $sum_print_price *= $this->discount_rate;
        $sum_output_price *= $this->discount_rate;
        $sell_price = $sum_paper_price + $sum_print_price + $sum_output_price;

        $result = [];
        $result["sell_price"] = $sell_price;
        $result["paper"] = $sum_paper_price;
        $result["print"] = $sum_print_price;
        $result["output"] = $sum_output_price;
        return $result;
    }
